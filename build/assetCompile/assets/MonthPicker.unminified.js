...
(function($, window, document, undefined) {
    ...
    $.widget("KidSysco.MonthPicker", {
        ...
        _showIcon: function() {
            if (this._monthPickerButton === null) {
                if (this.options.ShowIcon) {
                    this._monthPickerButton = $('<span id="MonthPicker_Button_' + this.element.attr('id') + '" class="month-picker-open-button nowrap">' + '</span>').insertAfter(this.element);
        ...
        _chooseYear: function(year) {
            ...
            // Calling showMonths to reload months and disabled by range.
            this._showMonths();
        },
        ...
        _showMonths: function() {
            ...
            this._toggleButton('.previous-year button', (this.options.MinDate && this._getPickerYear() - 1 < this.options.MinDate.getFullYear()));

            $('.next-year button', this._monthPickerMenu)
                .attr('title', this._i18n('nextYear'))
                .unbind('click')
                .bind('click.MonthPicker', $.proxy(this._nextYear, this));
            this._toggleButton('.next-year button', (this.options.MaxDate && this._getPickerYear() + 1 > this.options.MaxDate.getFullYear()));
            ...
            for (var _month in _months) {
                ...
                if (this._isYearOutsideOfRange(this._getPickerYear())) {
                    this._toggleButton('.button-' + _counter, true);
                } else if (this._isMonthOutsideOfRange(_month)) {
                    this._toggleButton('.button-' + _counter, true);
                } else {
                    this._toggleButton('.button-' + _counter, false);
                }
            }
        },
        ...
        _showYears: function() {
            ...
            this._toggleButton('.previous-year button', (this.options.MinDate && (_year + _yearDifferential) < this.options.MinDate.getFullYear()));
            ...
            this._toggleButton('.next-year button', (this.options.MinDate && (_year + 7) > this.options.MaxDate.getFullYear()));
            ...
            for (var _counter = 1; _counter <= 12; _counter++) {
                ...
                _currentYear = _year + _yearDifferential;
                $('.button-' + _counter, this._monthPickerMenu).button('option', 'label', _year + _yearDifferential);

                this._toggleButton('.button-' + _counter, this._isYearOutsideOfRange(_currentYear));
        ...
        _toggleButton: function(buttonName, disable) {
            $(buttonName).prop('disabled', disable);

            if (disable) {
                $(buttonName).addClass('month-picker-disabled-links');
            } else {
                $(buttonName).removeClass('month-picker-disabled-links');
            }
        },

        _isYearOutsideOfRange: function(currentYear) {
            return (this.options.MinDate && currentYear < this.options.MinDate.getFullYear() || this.options.MaxDate && currentYear > this.options.MaxDate.getFullYear());
        },

        _isMonthOutsideOfRange: function(currentMonth) {
            return (this.options.MinDate && this._getPickerYear() === this.options.MinDate.getFullYear() && currentMonth < this.options.MinDate.getMonth() || this.options.MaxDate && this._getPickerYear() === this.options.MaxDate.getFullYear() && currentMonth > this.options.MaxDate.getMonth())
        }
    });
